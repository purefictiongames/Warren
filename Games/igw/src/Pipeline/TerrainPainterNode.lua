--[[
    ...
    Performance: spatial grid bucketing (32x32), squared-distance early reject,
    deferred sqrt, numeric for-loops, time-based yielding.

    The height field computation is extracted into computeHeightField() so both
    Roblox (this handler) and Lune (mapgen adapter) can call the same code.
...
--------------------------------------------------------------------------------
-- HELPERS (module-level — used by computeHeightField + voxel writer)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- COMPUTE HEIGHT FIELD (pure math — callable from both Roblox + Lune)
--------------------------------------------------------------------------------
--
-- Parameters:
--   splines    — array of spline tables from SplinePlannerNode
--   biomeConfig — resolved biome config (baseElevation, spine, features)
--   opts       — {
--     mapWidth, mapDepth, groundY,
--     noiseAmplitude, noiseScale1, noiseScale2, noiseRatio,
--     seed,
--     yield  — optional function (e.g. task.wait) called periodically for yielding
--     noise  — optional noise function (default: math.noise; Lune passes Perlin.noise2d)
--   }
--
-- Returns:
--   { heightField, gridW, gridD, mapMinX, mapMinZ, maxH, spawn }

local function computeHeightField(splines, biomeConfig, opts)
    local mapWidth = opts.mapWidth or 4000
    local mapDepth = opts.mapDepth or 4000
    local groundY  = opts.groundY or 0

    local noiseAmplitude = opts.noiseAmplitude or 0
    local noiseScale1    = opts.noiseScale1 or 200
    local noiseScale2    = opts.noiseScale2 or 80
    local noiseRatio     = opts.noiseRatio or 0.65

    local yieldFn = opts.yield    -- nil on Lune, task.wait on Roblox
    local noiseFn = opts.noise or noise  -- Perlin.noise2d on Lune, math.noise on Roblox
    ...
    ----------------------------------------------------------------
    -- Spatial grid for segment bucketing (32x32)
    ----------------------------------------------------------------
    ...
    -- Noise seed offset
    local noiseSeed = (opts.seed or 0) * 0.01337
    ...
    for xi = 1, gridW do
        ...
        for zi = 1, gridD do
            ...
            if noiseAmplitude > 0 then
                local n1 = noiseFn(wx / noiseScale1 + noiseSeed, wz / noiseScale1 + noiseSeed)
                local n2 = noiseFn(wx / noiseScale2 + noiseSeed + 100, wz / noiseScale2 + noiseSeed + 100)
        ...
        -- Time-based yielding (~33ms budget per frame)
        if yieldFn and os.clock() - lastYield > 0.033 then
            lastYield = os.clock()
            yieldFn()
        end
    end
...
return {
    ...
    -- Exposed for Lune mapgen adapter (one codebase, different run targets)
    ...
    In = {
        onPaintTerrain = function(self, payload)
            ...
            task.spawn(function()
                ...
                -- If VPS provided maxH but no heightField, we regenerate
                -- the height field locally (deterministic from seed + splines).
                -- The VPS maxH is used for voxel buffer sizing.
                ...
                -- Terrain material from biome (Roblox-specific)
                ...
                print(string.format(
                    "[TerrainPainterNode] Spawn set at (0, %.0f, 0)",
