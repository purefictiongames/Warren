...
function TimedEvaluatorModule.initialize(config)
	...
	local function handleEnable()
		-- Debug: Log part states BEFORE showModel
		local partStates = {}
		for _, desc in ipairs(model:GetDescendants()) do
			if desc:IsA("BasePart") and desc.Name ~= "Anchor" then
				table.insert(partStates, desc.Name .. "(T:" .. desc.Transparency .. ",VA:" .. tostring(desc:GetAttribute("VisibleTransparency")) .. ")")
			end
		end
		System.Debug:Message(assetName, "BEFORE showModel - parts:", table.concat(partStates, ", "))

		Visibility.showModel(model)

		-- Debug: Log part states AFTER showModel
		partStates = {}
		for _, desc in ipairs(model:GetDescendants()) do
			if desc:IsA("BasePart") and desc.Name ~= "Anchor" then
		...
		System.Debug:Message(assetName, "AFTER showModel - parts:", table.concat(partStates, ", "))
