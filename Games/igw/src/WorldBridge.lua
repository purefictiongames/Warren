--[[
    IGW v2 — WorldBridge (Chunked World Manager)
    ...
    path with: buildWorld → WorldBridge → per-chunk DungeonOrchestrator dispatch.
...
return {
    ...
    _callVPS = function(self, action, rpcPayload)
        ...
        if not ok then
            error("[WorldBridge] VPS unreachable: " .. tostring(response))
        end
        if not response.Success then
            error("[WorldBridge] VPS returned HTTP " .. response.StatusCode)
        end
    ...
    _loadChunks = function(self, coordsList)
        ...
        if Debug then
            Debug.info("WorldBridge", "Loading", #toFetch, "chunks:", table.concat(toFetch, " "))
        end
        ...
        if not result or result.status ~= "ok" then
            warn("[WorldBridge] getChunks failed: " .. tostring(result and result.reason))
            return
        end
    ...
    _cutBorderDoors = function(self, doors)
        ...
        if hideCount > 0 or trussCount > 0 then
            print(string.format("[WorldBridge] Cut %d border door walls, placed %d trusses (%d doors)",
    ...
    _carveOverlapRooms = function(self, chunkKey, chunkData)
        ...
        if overlapCount > 0 then
            print(string.format("[WorldBridge] Carved %d overlap rooms for chunk %s",
    ...
    _unloadChunk = function(self, key)
        ...
        local Debug = _G.Warren.System.Debug
        if Debug then
            Debug.info("WorldBridge", "Unloaded chunk:", key)
        end
    end,
    ...
    _startBackgroundLoading = function(self)
        ...
        task.spawn(function()
            ...
            print(string.format("[WorldBridge] Background loading complete: %d chunks", loadedCount))
    ...
    In = {
        onBuildWorld = function(self, data)
            ...
            task.spawn(function()
                ...
                if Debug then
                    Debug.info("WorldBridge", "Building world:", biomeName, "compute:", computeTarget)
                end
                ...
                if not createResult or createResult.status ~= "ok" then
                    warn("[WorldBridge] world.create failed: " .. tostring(createResult and createResult.reason))
                    return
                end
                ...
                if Debug then
                    Debug.info("WorldBridge", "Loading initial chunks around spawn",
                ...
                print(string.format(
                    "[WorldBridge] World ready: %s | %d chunks loaded (%.2fs)",
