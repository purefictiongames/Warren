...
System:RegisterAsset(assetName, function()
	...
	local function getConfig()
		return {
			...
			tentTemplate = model:GetAttribute("TentTemplate") or "Dropper", -- Template spawned by ArrayPlacer
		}
	end
	...
	-- Pattern: {ArrayPlacerAlias}_{TentTemplate}_{index}
	local function refreshTentList()
		availableTents = {}
		local config = getConfig()
		for i = 1, config.activeTents do
			local tentName = config.targetArrayPlacer .. "_" .. config.tentTemplate .. "_" .. i
