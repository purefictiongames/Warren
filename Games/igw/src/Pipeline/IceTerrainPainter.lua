--[[
    ...
    Signals:
      onBuildPass — original pipeline path (fires buildPass)
      onPaintMountainRooms — Phase 2 alias (fires nodeComplete)
--]]
...
return {
    ...
    In = {
        onBuildPass = function(self, payload)
            ...
            self:_paintRooms(payload)
            self.Out:Fire("buildPass", payload)
        end,

        onPaintMountainRooms = function(self, payload)
            self:_paintRooms(payload)
