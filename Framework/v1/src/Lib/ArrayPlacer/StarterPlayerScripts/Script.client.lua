...
local function createDisplay(model)
	...
	-- Find anchor - check direct child first, then search descendants
	local anchor = model:FindFirstChild("Anchor")
	if not anchor then
		for _, desc in ipairs(model:GetDescendants()) do
			if desc.Name == "Anchor" and desc:IsA("BasePart") then
				anchor = desc
				break
			end
		end
	...
	if not anchor then
		System.Debug:Warn(assetName .. ".client", "No anchor found for", model.Name)
		return
	end

	System.Debug:Message(assetName .. ".client", "Found anchor for", model.Name, "at", anchor:GetFullName())
	...
	-- Debug: Log initial HUDVisible state
	local initialHUDVisible = model:GetAttribute("HUDVisible")
	System.Debug:Message(assetName .. ".client", "Initial HUDVisible for", spawnName, "=", initialHUDVisible, "Billboard enabled:", billboardGui.Enabled)

	-- Debug: Verify client-side part states
	local clientPartStates = {}
	for _, desc in ipairs(model:GetDescendants()) do
		if desc:IsA("BasePart") and desc.Name ~= "Anchor" then
			table.insert(clientPartStates, desc.Name .. "(T:" .. desc.Transparency .. ",Pos:" .. math.floor(desc.Position.Y) .. ")")
		end
	end
	System.Debug:Message(assetName .. ".client", "CLIENT parts:", table.concat(clientPartStates, ", "))

	-- Debug: Log anchor position for billboard
	System.Debug:Message(assetName .. ".client", "Anchor position:", anchor.Position, "Billboard studsOffset:", billboardGui.StudsOffset)
