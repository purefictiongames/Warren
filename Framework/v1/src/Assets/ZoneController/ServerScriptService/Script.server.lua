...
local function setupZoneController(model)
    ...
    if not matchCallback then
        warn("ZoneController: No MatchCallback attribute set on", model.Name)
        return
    end
    ...
    if not zone then
        warn("ZoneController: No Zone part found in", model.Name)
        return
    end
    ...
    print("ZoneController: Set up", model.Name, "(MatchCallback:", matchCallback, ", TickRate:", tickRate, ")")
end
